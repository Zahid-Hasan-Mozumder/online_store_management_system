import { ConflictException, HttpException, Injectable, InternalServerErrorException, NotFoundException, UnprocessableEntityException } from '@nestjs/common';
    ...
    async createVarient(productId: number, dto: VarientsDto) {
        ...
        try {
            const result = await this.prisma.$transaction(async (tx) => {
                ...
                const varient = await tx.varients.create({
                    data: {
                        title: dto.title,
                        option1: dto.option1,
                        option2: dto.option2,
